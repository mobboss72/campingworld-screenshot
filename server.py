...
def generate_pdf(
    ...
    try:
        ...
        def measure_hash_height():
            ...
            h += 0.2 * inch  # Change from 0.1 to 0.2
            # Heading
            h += 0.16 * inch
            ...
            if sha_price and sha_price != "N/A":
                ...
                h += 0.04 * inch  # Change from 0.12 to 0.04
            if sha_pay and sha_pay != "N/A":
        ...
        footer_needed = hashes_h + 0.7 * inch  # Change from 0.4 to 0.7
        ...
        # Draw SHA section
        y = sha_section_top
        y += 0.2 * inch  # Change from 0.1 to 0.2
        c.setFont("Helvetica-Bold", 10)
        ...
        def draw_hash(label, value, y):
            ...
            y -= used_h
            y -= 0.04 * inch  # Change from 0.12 to 0.04
            return y
...
def admin_backfill_tsa(capture_id):
    try:
        ...
        # ---- Sign page capture (added) ----
        sign_path = None
        try:
            base_dir = os.path.dirname(price_path) if price_ok else (os.path.dirname(pay_path) if pay_ok else tempfile.mkdtemp(prefix=f"cw-sign-{stock}-"))
            sign_path = capture_sign_builder_image(stock, base_dir)
        except Exception:
...
@app.post("/capture")
def capture():
    prune_captures_retention(365)  # retention enforcement
    try:
...
def prune_captures_retention(days:int=365):
    """
    Best-effort prune for 'captures' table (older than 'days' by created_at).
    Safe to call even if table/column don't exist.
    """
    try:
        with get_db() as conn:
            conn.execute("DELETE FROM captures WHERE created_at < datetime('now', ?)", (f'-{days} days',))
    except Exception:
        pass
